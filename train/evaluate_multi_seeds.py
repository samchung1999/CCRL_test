...
class Evaluator:
    def __init__(self, config):
        ...
        if self.model_index:
            print("load model...")
            # 自动将模型加载到当前默认设备，无论是 CPU 还是第一个可用的 GPU
            self.net.load_state_dict(torch.load(model_path, map_location=torch.device('cuda:0' if torch.cuda.is_available() else 'cpu')))
